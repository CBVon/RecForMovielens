...
class SVD():  
    ...
    def initb(self):
	#print [i for i in range(self.userNum)]
	f=open(self.allfile) 
	a = f.readlines()
	...
	self.numu=[0 for i in range(self.userNum)]  
        self.numi=[0 for i in range(self.itemNum)]
	for x in b:
	    #print x[0],x[1],x[2]
	    #print self.bu
	    self.bu[x[0] - 1] += (x[2] - self.av)
	    self.numu[x[0] - 1] += 1
	    self.bi[x[1] - 1] += (x[2] - self.av)
	    self.numi[x[1] - 1] += 1
	for x in range(self.userNum):
	    self.bu[x - 1] /= self.numu[x - 1]
	    self.bi[x - 1] /= self.numi[x - 1]
