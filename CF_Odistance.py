...
def sim_distance(prefer, person1, person2): 
    ...
    # compute the "suqare of differece" between the prefer of person1, 2   
...
def topKMatches(prefer, person, itemId, k=20, sim = sim_distance):
    userSet = []
    scores = []
    users = []
...
def getRating(prefer1, userId, itemId, knumber=20,similarity=sim_distance):
    sim = 0.0
    averageOther =0.0
    jiaquanAverage = 0.0
    simSums = 0.0
    #  users for saving topK neighbour who has prefered  
    users = topKMatches(prefer1, userId, itemId, k=knumber, sim = sim_distance)
...
# get all "prediction prefer" of users in testfile, and save it in "resultO.txt" 
def getAllUserRating(fileTrain='u1.base', fileTest='u1.test', fileResult='resultO.txt', similarity=sim_distance):
...
# sort "resultO.txt" by user_first, item_second
def sort():
    f=open('resultO.txt') 
    a = f.readlines()
    ...
    f=open('resultO.txt', "w")   
    f.writelines(b)  
    f.close()    

# get Rese between "resultO.txt" and real "u1.test"
def getRmse():
    f1 = open('resultO.txt')
    a1 = f1.readlines()
...
def Odraw():
    ...
    plt.scatter(xx, b1, label = 'resultO')
    ...
    #x = range(len(self.arrRMSE_test))
    plt.scatter(xx, b2, label = 'u1.test')

    plt.legend()
    plt.show()
    plt.savefig('resultO.png')

if __name__ == "__main__":
    start = time.clock()
    print("\n--------------Movielens dataset——userbased CF(Odistance)--------------\n")
    if os.path.exists('resultO.txt'):   	
	os.remove('resultO.txt')
    getAllUserRating('u1.base', 'u1.test', 'resultO.txt')
    sort()
    Rmse = getRmse()
    print("RMSE(Odistance) is : ", Rmse)
    f = open('resultO.txt', "a+")
    f.write("RMSE(Odistance) is : " + str(Rmse) + "\n")
    #Odraw()
    #f.write(str(Rmse))
    end = time.clock()
